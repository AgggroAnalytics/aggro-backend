-- PostGIS, enums, fields, tiles, timeseries, predictions, analytics (depends on organizations from 000001)
...
CREATE TABLE IF NOT EXISTS field_analytics_timeseries (
  ...
  CONSTRAINT field_analytics_tile_count_nonnegative CHECK (tile_count IS NULL OR tile_count >= 0),
  CONSTRAINT field_analytics_valid_tile_count_nonnegative CHECK (valid_tile_count IS NULL OR valid_tile_count >= 0),
...
CREATE INDEX IF NOT EXISTS idx_tiles_field_id ON tiles (field_id);
CREATE INDEX IF NOT EXISTS idx_tiles_geometry ON tiles USING GIST (geometry);
CREATE INDEX IF NOT EXISTS idx_tile_timeseries_tile_id_observation_date ON tile_timeseries (tile_id, observation_date DESC);
CREATE INDEX IF NOT EXISTS idx_tile_predictions_tile_id_prediction_date ON tile_predictions (tile_id, prediction_date DESC);
CREATE INDEX IF NOT EXISTS idx_field_analytics_field_id_observation_date ON field_analytics_timeseries (field_id, observation_date DESC);
CREATE INDEX IF NOT EXISTS idx_field_predictions_field_id_prediction_date ON field_predictions (field_id, prediction_date DESC);
