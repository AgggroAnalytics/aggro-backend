...
const deletePredictedFieldAnalyticsByFieldID = `-- name: DeletePredictedFieldAnalyticsByFieldID :exec
DELETE FROM field_analytics_timeseries
WHERE field_id = $1 AND source = 'predicted'
`

func (q *Queries) DeletePredictedFieldAnalyticsByFieldID(ctx context.Context, fieldID uuid.UUID) error {
	_, err := q.db.Exec(ctx, deletePredictedFieldAnalyticsByFieldID, fieldID)
	return err
}
...
func (q *Queries) ListFieldAnalyticsByFieldID(ctx context.Context, arg ListFieldAnalyticsByFieldIDParams) ([]ListFieldAnalyticsByFieldIDRow, error) {
	...
	for rows.Next() {
		...
		if err := rows.Scan(
			...
			&i.PredictionVegetationActivityDrop,
			&i.PredictionHeterogeneityGrowth,
			&i.PredictionIrrigationEventsDetected,
			&i.CreatedAt,
		); err != nil {
			return nil, err
		}
...
func (q *Queries) UpsertFieldPredictedAnalyticsForFieldAndDate(ctx context.Context, arg UpsertFieldPredictedAnalyticsForFieldAndDateParams) error {
	_, err := q.db.Exec(ctx, upsertFieldPredictedAnalyticsForFieldAndDate,
		...
		arg.PredictionVegetationActivityDrop,
		arg.PredictionHeterogeneityGrowth,
		arg.PredictionIrrigationEventsDetected,
	)
	return err
}
